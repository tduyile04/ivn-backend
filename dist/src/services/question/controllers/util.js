...
exports.fmtRawResponse = fmtRawResponse;
exports.fetchCandidate = fetchCandidate;
...
function isExist(col, list) {
  return col && list.filter(function (l) {
    return l.id === col;
  }).length === 0;
}

function fmtRawResponse(data) {
  var keep = ['id', 'likes', 'comments', 'answers', 'question', 'asker', 'created_at', 'candidate'];
  return Object.values(data.reduce(function (acc, row) {
    if (acc[row.id]) {
      isExist(row.like_id, acc[row.id].likes) && acc[row.id].likes.push({ id: row.like_id, user: genUser('like', row) });
      isExist(row.answer_id, acc[row.id].answers) && acc[row.id].answers.push({ id: row.answer_id, answer: row.answer, user: genUser('candidate', row) });
      isExist(row.comment_id, acc[row.id].comments) && acc[row.id].comments.push({ id: row.comment_id, comment: row.comment, user: genUser('comment', row) });
    } else {
      acc[row.id] = row;
      acc[row.id].asker = genUser('asker', row);
      acc[row.id].candidate = genUser('candidate', row);
      acc[row.id].answers = row.answer_id ? [{ id: row.answer_id, answer: row.answer, user: genUser('candidate', row) }] : [];
      acc[row.id].comments = row.comment_id ? [{ id: row.comment_id, comment: row.comment, user: genUser('comment', row) }] : [];
      acc[row.id].likes = row.like_id ? [{ id: row.like_id, user: genUser('like', row) }] : [];
      Object.keys(acc[row.id]).forEach(function (r) {
        if (keep.indexOf(r) === -1) {
          delete acc[row.id][r];
        }
      });
...
function fetchCandidate(id) {
  return (0, _models2.default)('user as u').leftJoin('user_role as ur', 'u.id', 'ur.user_id').leftJoin('role as r', 'ur.role_id', 'r.id').select(['u.id', 'r.name as roleName']).where(_models2.default.raw('u.id = ?', [id])).then(function (result) {
    if (result.length === 0) {
      throw new Error();
    } else if (result.filter(function (r) {
      return r.roleName === 'candidate';
    }).length === 0) {
      throw new Error();
    }
    return true;
