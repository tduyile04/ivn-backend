...
exports.default = function () {
  ...
  return (0, _waterfall2.default)(args, [checkBody, validate, findUser, isAdmin, findRole, findUserRole, removeRole, fmtResult]);
};
...
function findUserRole(data, res, callback) {
  return (0, _models2.default)('user_role').where({ user_id: data.user.id, role_id: data.role.id }).select(['id']).then(function (results) {
    return results.length > 0 ? callback(null, Object.assign({}, data, { userRole: results[0].id }), res) : callback({ message: 'User does not have this role', code: 400 }); // eslint-disable-line
  }).catch(function (e) {
...
function removeRole(data, res, callback) {
  return (0, _models2.default)('user_role').where({ id: data.userRole }).del().then(function () {
    return callback(null, data, res);
  }).catch(function (e) {
    return (0, _error.errorHandler)(e, res);
  });
